...
#[test]
fn test_create_good_uci_command_with_spaces() {
    let r = Command::parse("     uci    ");
    assert!(r.is_ok());
...
#[test]
fn test_create_good_uci_command_but_with_extra_params() {
    let r = Command::parse("uci param1 param2");
    assert!(r.is_err());
}
